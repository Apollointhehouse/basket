package kyta.composter.protocol.packet.play

import kyta.composter.protocol.PacketHandler
import kyta.composter.protocol.io.PacketSerializer
import kyta.composter.protocol.io.ReadBuffer
import kyta.composter.protocol.ServerboundPacket

data class C2SCloseMenuPacket(val id: Int): ServerboundPacket {
    override suspend fun handle(handler: PacketHandler) = handler.handleMenuClose(this)

    companion object : PacketSerializer<C2SCloseMenuPacket> {
        override fun deserialize(buffer: ReadBuffer): C2SCloseMenuPacket {
            return C2SCloseMenuPacket(buffer.readByte().toInt())
        }
    }
}